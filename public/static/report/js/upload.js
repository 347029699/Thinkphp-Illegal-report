 ...
 // alert(123);
        ...
        $('.uploder-container').each(function(index){
            var self=$(this);
            var my_id=self.find(".jxfilePicker").data("id");
            // console.log(my_id);
            // 添加的文件数量
            var fileCount = 0;
            // 添加的文件总大小
            ...
            // 图片容器         
            var queue = $('<ul class="filelist"></ul>').prependTo(queueList);

             var url = "http://"+window.location.host+"/service/index/addorder";
           //var url = "";
            ...
            uploader[index] = WebUploader.create({
                ...
                accept: {
                    title: 'Images',
                    extensions: 'gif,jpg,jpeg,png,mp3,mp4',
                    mimeTypes: '*'
                },
                ...
                fileSingleSizeLimit: 30 * 1024 * 1024 ,   // 单文件的最大限制30M
            ...
            uploader[index].on('uploadSuccess',function(file,response){
                ...
                console.log(uploader[index]);

                //往input 传值



                $("#uploader").append("<input type='hidden' name='images["+my_id+"][]' value='"+response.id +"'/>");
