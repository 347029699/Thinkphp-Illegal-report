...
jQuery(document).ready(function() {
    ...
    $('.js-jcrop-interface').each(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.code == 1) {
                                ...
                                Ma.loading('hide');
                                Ma.notify(res.msg||'上传失败，请重新上传', 'warning');
                            }
                        }
                    }).fail(function(res) {
                        Ma.loading('hide');
        ...
        $jcrop_cut_btn.click(function () {
            ...
            if ($cut_value != '') {
                ...
                .fail(function(res) {
                    Ma.loading('hide');
                    Ma.notify($(res.responseText).find('h1').text() || '请求失败~', 'danger');
                });
            } else {
                Ma.notify('请选择要裁剪的大小', 'warning');
            }
        });
    ...
    $('.js-ueditor').each(function () {
        ...
        ueditors[ueditor_name] = UE.getEditor(ueditor_name, {
            ...
            serverUrl: ma.ueditor_upload_url
        });
    });
    ...
    $('.js-wangeditor').each(function () {
        ...
        // 上传图片地址
        wangeditors[wangeditor_name].config.uploadImgUrl = ma.wangeditor_upload_url;
        // 允许上传图片后缀
    ...
    $('.js-upload-file,.js-upload-files').each(function () {
        ...
        var uploader = WebUploader.create({
            ...
            duplicate: true,
            // swf文件路径
            swf: ma.WebUploader_swf,
            // 文件接收服务端。
            server: ma.file_upload_url,
            // 选择文件的按钮。可选。
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Ma.notify('文件类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Ma.notify('文件不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.js-upload-filebig').each(function () {
        var $input_file = $(this).find('input');
        var $input_file_name = $input_file.attr('name');
        // 是否多文件上传
        var $multiple = $input_file.data('multiple');
        ...
        var $file_list = $('#file_list_' + $input_file_name);
        // 单文件文件限制大小
        var $chunksize = $input_file.data('chunksize');
        ...
        var uploader = WebUploader.create({
            ...
            swf: ma.WebUploader_swf,
            // 文件接收服务端。
            server: ma.file_upload_url+'?bigfile=1',
            // 选择文件的按钮。可选。
            // 内部根据当前运行是创建，可能是input元素，也可能是flash.
            pick: {
                id: '#picker_' + $input_file_name,
    ...
    $('.js-upload-image,.js-upload-images').each(function () {
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Ma.notify('图片类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Ma.notify('图片不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.select-linkage').change(function(){
        ...
        if (value !== '') {
            ...
            .fail(function(res) {
                Ma.loading('hide');
                Ma.notify($(res.responseText).find('h1').text() || '数据请求失败~', 'danger');
            });
        }
    });
    ...
    if (ma.triggers != '') {
        /* 依赖显示 */
        // 先隐藏依赖项
        var $field_hide   = ma.field_hide.split(',') || [];
        var $field_values = ma.field_values.split(',') || [];
        ...
        $.each(ma.triggers, function (trigger, content) {
                ...
                $(content).each(function () {
                    ...
                    if ($.inArray($value, $values) >= 0) {
                        ...
                        for (var item in $targets) {
                            ...
                            if (ma._field_clear[trigger] !== undefined && ma._field_clear[trigger] === 1) {
