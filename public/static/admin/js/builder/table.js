...
jQuery(document).ready(function() {
    if ($.fn.editable) {
        // 快速编辑的url提交地址
        $.fn.editable.defaults.url = ma.quick_edit_url;
        // 值为空时显示的信息
        ...
        $.fn.editable.defaults.params = function (params) {
            ...
            params.validate = ma.validate;
            params.validate_fields = ma.validate_fields;
            return params;
        };
        // 提交成功时的回调函数
    ...
    var goto = function (url, _curr_params, remove_page) {
        ...
        if (remove_page && ma.curr_params['page'] !== undefined) {
            delete ma.curr_params['page'];
        }

        if ($.isEmptyObject(ma.curr_params)) {
            params = jQuery.param(_curr_params);
        } else {
            $.extend(ma.curr_params, _curr_params);
            params = jQuery.param(ma.curr_params);
        }
    ...
    // 初始化搜索
    var search_field = ma.search_field;
    ...
    $('.table-builder .field-filter').click(function () {
        var self             = $(this),
            ...
            $_filter         = ma._filter,
            $_filter_content = ma._filter_content,
            $_field_display  = ma._field_display,
            $data  = {
        ...
        layer.open({
            ...
            success: function () {
                ...
                // 获取数据
                $.post(ma.get_filter_list, $data).success(function(res) {
                    if (1 !== res.code) {
            ...
            yes: function () {
                ...
                goto(ma.curr_url, _curr_params, true);
            }
        });
        return false;
    });
    ...
    $('.table-builder .switch input:checkbox').on('click', function () {
        ...
        // 发送ajax请求
        Ma.loading();
        $.post(ma.quick_edit_url, $data).success(function(res) {
            Ma.loading('hide');
            if (res.code) {
                Ma.notify(res.msg, 'success');
            } else {
    ...
    $('#page-container').delegate('a.pop', 'click', function () {
        ...
        // 处理各种回调方法
        ma.layer.success = ma.layer.success ? window[ma.layer.success] : null;
        ma.layer.yes     = ma.layer.yes ? window[ma.layer.yes] : null;
        ma.layer.cancel  = ma.layer.cancel ? window[ma.layer.cancel] : null;
        ma.layer.end     = ma.layer.end ? window[ma.layer.end] : null;
        ma.layer.full    = ma.layer.full ? window[ma.layer.full] : null;
        ma.layer.min     = ma.layer.min ? window[ma.layer.min] : null;
        ma.layer.max     = ma.layer.max ? window[ma.layer.max] : null;
        ma.layer.restore = ma.layer.restore ? window[ma.layer.restore] : null;

        if ($layer !== undefined) {
            // 处理各种回调方法
            $layer.success = $layer.success ? window[$layer.success] : ma.layer.success;
            $layer.yes     = $layer.yes ? window[$layer.yes] : ma.layer.yes;
            $layer.cancel  = $layer.cancel ? window[$layer.cancel] : ma.layer.cancel;
            $layer.end     = $layer.end ? window[$layer.end] : ma.layer.end;
            $layer.full    = $layer.full ? window[$layer.full] : ma.layer.full;
            $layer.min     = $layer.min ? window[$layer.min] : ma.layer.min;
            $layer.max     = $layer.max ? window[$layer.max] : ma.layer.max;
            $layer.restore = $layer.restore ? window[$layer.restore] : ma.layer.restore;

            $.extend($options, ma.layer, $layer);
        } else {
            $.extend($options, ma.layer);
        }
