...
use app\common\model\Txt as TxtModel;
...
if (!function_exists('intergration_log')) {
    /**
     * 记录积分日志
     * @param null $action 行为标识
     * @param null $user_id 执行行为的用户id
     ...
     * @return bool|string
     */
    function intergration_log($action = null, $user_id = null, $type = 0,$mode = 0,$mode_id = 0)
    {
        // 参数检查
        if (empty($action)) {
            return '参数不能为空';
        ...
        // 查询行为,判断是否执行
        $action_info = model('user/integral')->where($where)->find();
        ...
        $data = [
            ...
            'action_ip' => get_client_ip(1),
            'create_time' => request()->time(),
            'remark' => $nickname . ' ' . $action . ':' . $action_info['msg'] . $action_info['number'] . $field,
            'type' => $type,
            'mode' => $mode,
            'mode_id' => $mode_id,
            'status' => '1',
...
if (!function_exists('logWrite')) {
    function logWrite($filename='',$str,$fil='log',$model='w+'){
        TxtModel::logWrite($filename,$str,$fil,$model);
    }
}
...
if (!function_exists('get_file_ext')) {
    ...
    {
        $name = model('admin/attachment')->getFileExt($id);
        if (!$name) {
            return '没有找到文件';
        }
        return $name;
