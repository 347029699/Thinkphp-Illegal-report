...
class Attachment extends Admin
{
    ...
    public function index()
    {
        ...
        foreach ($data_list as $key => &$value) {
            ...
            if ($value['driver'] == 'local') {
                $data_list[$key]['path'] = $this->request->domain().PUBLIC_PATH. $value['path'];
            }
        }
    ...
    public function add()
    {
        $data = array();
        $fields = [
            ['name' => 'file1', 'title' => '上传图片', 'type' => 'file', 'value' => ''],
        ];
        // 使用ZBuilder快速创建表单
     ...
     * @return mixed
     */
    public function upload($dir = '', $from = '', $module = '',$bigfile=0)
    {
        if($bigfile== 1){
            return $this->uploadbig($dir, $from, $module);
        }
        // 临时取消执行时间限制
        set_time_limit(0);
    ...
    public function uploadbig($dir = '', $from = '', $module = '')
    {
        $folder = $this->request->file('file');
        ...
        usleep(5000);
        // Settings
        $targetDir = './uploads'.DIRECTORY_SEPARATOR.'tmp';            //存放分片临时目录
        //存放点
        $uploadDir = './uploads'.DIRECTORY_SEPARATOR.'bigfile'.DIRECTORY_SEPARATOR.date('Ymd');
        ...
        if ($cleanupTargetDir) {
            if (!is_dir($targetDir) || !$dir = opendir($targetDir)) {
                return json([
                    'code'   => 0,
                    'class'  => 'danger',
                    'info'   => 'Failed to open temp directory111.'
                ]);
            }
        ...
        if (!$out = fopen("{$filePath}_{$chunk}.parttmp", "wb")) {
            return json([
                'code'   => 0,
                'class'  => 'danger',
                'info'   => 'Failed to open output stream222.'
            ]);
        }
        if (!empty($_FILES)) {
            if ($_FILES["file"]["error"] || !is_uploaded_file($_FILES["file"]["tmp_name"])) {
                return json([
                    'code'   => 0,
                    'class'  => 'danger',
                    'info'   => 'Failed to move uploaded file333.'
                ]);
            }
            ...
            if (!$in = fopen($_FILES["file"]["tmp_name"], "rb")) {
                return json([
                    'code'   => 0,
                    'class'  => 'danger',
                    'info'   => 'Failed to open input stream444.'
                ]);
            }
        } else {
            if (!$in = fopen("php://input", "rb")) {
                return json([
                    'code'   => 0,
                    'class'  => 'danger',
                    'info'   => 'Failed to open input stream555.'
                ]);
            }
        }
        ...
        if ($done) {
            ...
            if (!$out = fopen($uploadPath, "wb")) {
                return json([
                    'code'   => 0,
                    'class'  => 'danger',
                    'info'   => 'Failed to open output stream666.'
                ]);
            }
...
//                'file_id'=>$data['id'],
            ...
            $file_info = [
                ...
                'name'   => $oldName,
                'mime'   => $folder->getInfo('type'),
                'path'   => $uploadPath,
                'ext'    => $pathInfo['extension'],
                'size'   => $folder->getInfo('size'),
                'md5'    => '',
                'sha1'   => '',
                'thumb'  => '',
                'module' => $module,
