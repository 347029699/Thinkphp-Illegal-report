...
class Index extends Admin
{
    ...
    public function index()
    {
        ...
        $modpsd_time = Db::name('admin_user')->where('id', $userinfo['uid'])->value('modpsd_time');
        //$modpsd_time2 = strtotime("+1 months", $modpsd_time);
        $modpsd_time3 = strtotime("-1 months", time());
        if ($modpsd_time <=  $modpsd_time3) {
            $this->assign('default_pass', 1);
        }
        ...
        if ($this->request->isPost()) {
            ...
            if(isset($data['password'])) {
                if (strlen($data['password']) < 8) {
                    $this->error('至少8位以上，由数字、字母两种方式组成');
                }
                $respsd = preg_match('/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]+$/', $data['password']);
                if (!$respsd) {
                    $this->error('至少8位以上，由数字、字母两种方式组成');
                }
                //密码修改时间
                $data['modpsd_time'] = time();
            }

            $UserModel = new UserModel();
            if ($user = $UserModel->allowField(['nickname', 'email', 'password', 'mobile', 'avatar','modpsd_time'])->update($data)) {
                // 记录行为
